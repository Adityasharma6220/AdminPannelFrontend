...
export default function TableComponent() {
    document.title = `Lottery Time | ${AppName}`
  const Column = [
    { id: 1, value: 'time', label: 'Time' },
    { id: 2, value: 'isActive', label: 'Status' },
    { id: 3, value: 'createdDate', label: 'Created Date' },
  ...
  const getUserDetails = async () => {
    ...
    try {
      let response = await axios.post(`${BackendURL}v2/lottery/lottery-time/get-lottery-time`, inputparameters, { headers: headers });
  ...
  const UpdateStatus = async (e, id) => {
    ...
    try {
      let response = await axios.post(`${BackendURL}v2/lottery/lottery-time/update-status-lottery-time`, inputparameters, { headers: headers });
      
      if (e.target.checked) {
        toast.success("Lottery time is activated !");
      } else {
        toast.success("Lottery time is deactivated !");
      }
    } catch (error) {
      if (error.response && error.response.status === 401) {
        localStorage.removeItem('token');
  ...
  return (
    <>
        <Paper>
        <MainCard title="Lottery Time">
        <Grid container spacing={gridSpacing} justifyContent="flex-end" alignItems="center" style={{marginBottom:"10px"}}>
        <Grid item>
         <Link to="/admin/lottery/lottery-time/add-lottery-time"><Button variant="contained" color="warning">Add Lottery Time</Button></Link> 
        </Grid>
        </Grid>
                ...
                {rows?.map((row) => (
                  <StyledTableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                    ...
                    <StyledTableCell>
                      ...
                      <Link to={`/admin/lottery/lottery-time/edit-lottery-time/${row._id}`}>
                        <IconEdit />
                        </Link>
                      </IconButton>
