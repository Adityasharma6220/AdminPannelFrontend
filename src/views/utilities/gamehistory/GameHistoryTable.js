...
import { Rows, BackendURL, AppName, gridSpacing } from '../../../store/constant';
...
export default function TableComponent({
  ...
  React.useEffect(() => {
    document.title = `Game History | ${AppName}`
  ...
  const getUserDetails = async (startDate, endDate) => {
    ...
    try {
      ...
      setLoading(false) // after fetch the data
    }
  };
      ...
      <Grid container direction="row" justifyContent="space-between" alignItems="flex-end">
        ...
        <Grid item xs={2}>
          <Box
            component="form"
            sx={{
              '& .MuiTextField-root': { m: 0.3, width: '40%' }
            }}
            noValidate
            autoComplete="off"
          >
                  ...
                  (
                    ...
                    rows?.map((row) => (
                      ...
                      <StyledTableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                        ...
                        <StyledTableCell>
                          <Link to={`/admin/users/view/${row.userId}`}>{row.userId}</Link>
                        </StyledTableCell>
                        <StyledTableCell>{row.gameMode}</StyledTableCell>
                        <StyledTableCell>{row.total_Bet_Value}</StyledTableCell>
                        <StyledTableCell>{row.total_Win}</StyledTableCell>
                        <StyledTableCell>{moment(row.createdDate).format('DD/MM/YYYY hh:mm A')}</StyledTableCell>
                        <StyledTableCell><Link to={`/admin/game-history/view/${row._id}`}><IconButton aria-label="fingerprint" color="success"><IconEye /></IconButton></Link></StyledTableCell></StyledTableRow>
                    ))
                  )
