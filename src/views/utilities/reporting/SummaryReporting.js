...
import {
  Card, Grid, Typography, CardContent, TextField,
  Box, Table, TableBody, TableContainer, TableHead, TableRow, MenuItem,
...
const SummaryReporting = () => {
  ...
  const getUserDetails = async (startDate, endDate) => {
    ...
    try {
      let response = await axios.post(`${BackendURL}reporting/getSummaryReportDateWise`, inputparameters, { headers: headers });
                            ...
                            (
                              ...
                              rows?.map((row) => (
                                <StyledTableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                                  ...
                                  <StyledTableCell>{(Math.floor(row?.TotalBetValue) % 1 === 0) ? row?.TotalBetValue.toFixed(2) : (Math.floor(row?.TotalBetValue)).toFixed(2)}</StyledTableCell>
                                  <StyledTableCell>{(Math.floor(row?.TotalWin) % 1 === 0) ? row?.TotalWin.toFixed(2) : (Math.floor(row?.TotalWin)).toFixed(2)}</StyledTableCell>
