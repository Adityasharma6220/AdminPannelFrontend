...
export default function TableComponent({
  ...
  React.useEffect(() => {
    document.title = `Dispute | ${AppName}`
    getUserDetails();
              ...
              <TableBody>
                {rows?.map((row) => (
                  <StyledTableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                    <StyledTableCell>
                      <Link to={`/admin/users/view/${row.userId}`}>{row.username}</Link>
                    </StyledTableCell>
                    <StyledTableCell><Link to={`/admin/game-history/view/${row.gameId}`}>{row.gameId}</Link></StyledTableCell>
