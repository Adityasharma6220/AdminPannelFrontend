...
export default function TableComponent({
  ...
  React.useEffect(() => {
    document.title = `Rewards | ${AppName}`
    getRewardsMethod();
  }, [page, rowsperpage, search, field, sortby]);

  const getRewardsMethod = async () => {
    const headers = {
  ...
  const DeleteReward = async (id) => {
    ...
    try {
      ...
      toast.success("Reward is deleted !");
      await getRewardsMethod();
    } catch (error) {
      if (error.response && error.response.status === 401) {
        localStorage.removeItem('token');
        navigate('/login');
              ...
              <TableBody>
                {rows?.map((row) => (
                  <StyledTableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                    ...
                    <StyledTableCell>{row.rewardamount}</StyledTableCell>
                    ...
                    <StyledTableCell>
                      ...
                      <IconButton aria-label="fingerprint" color="error" onClick={() => DeleteReward(row._id)}>
                        <IconTrash />
                      </IconButton>
                    </StyledTableCell>
                  </StyledTableRow>
