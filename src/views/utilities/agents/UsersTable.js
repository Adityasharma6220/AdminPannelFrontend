...
const Column1 = [
    ...
    { id: 7, value: 'isAddMoney', label: 'Is Money Added' },
...
export default function UsersTable() {
  ...
  const getAgentDetails = async () => {
    ...
    try {
      const response = await axios.post(`${BackendURL}agents/getUsersFromOperator`, inputparameters, { headers });
              ...
              <TableBody>
                {rows?.map((row) => (
                  <StyledTableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                    ...
                    <StyledTableCell>{row.isAddMoney ? "Yes" : "No"}</StyledTableCell> 
                    <StyledTableCell><IOSSwitch defaultChecked={row.isVerified}onChange={(e) => { UpdateStatus(e, row._id); }}/></StyledTableCell>
                    <StyledTableCell>{moment(row.createdDate).format('DD/MM/YYYY')}</StyledTableCell> 
                    <StyledTableCell>

                    <IconButton aria-label="fingerprint" color="success">
                        <Link to={`/admin/users/view/${row._id}`}>
                          <IconEye />
                        </Link>
                      </IconButton>
