...
import {Paper, Table,TableBody,TableContainer,Typography,TableHead,TableRow,Box,TextField,Button,MenuItem,Grid,Pagination,IconButton,Switch,Card,CardContent,styled} from '@mui/material';
...
export default function AgentsTable({_id}) {
  ...
  React.useEffect(() => {
    getAgentDetails();
    getUserDetails();
  }, [page, rowsPerPage, search, field, sortby,role]);
  ...
  const getAgentDetails = async () => {
    ...
    try {
      const response = await axios.post(`${BackendURL}agents/gettingAgents`, inputparameters, { headers });
  ...
  return (
    ...
    {
        role == 'agent' || role == 'apk_manger' ? <OperatorTable/> :  role == 'operator' || role == 'apk_dealer' ? <UsersTable/> : <Paper>
